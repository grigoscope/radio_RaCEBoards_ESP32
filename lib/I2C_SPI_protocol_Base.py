...
class I2C_Impl:
    ...
    def read_register_nonstop(self, register: int, length: int) -> bytearray:
        "Read from the device register, not generate stop"
        with self._i2c as i2c:
            in_buf = bytearray(length)
            i2c.write_then_readinto(bytes([register & 0xFF]),in_buf)
            return in_buf
