...
"""
    ...
    3D-vis: https://adafruit.github.io/Adafruit_WebSerial_3DModelViewer/
"""
...
class MadgwickAHRS:
    ...
    def update(self, gyroscope, accelerometer, magnetometer):
        ...
        :return:
        """
        timemns = time.monotonic_ns()
        self.samplePeriod = (timemns - self.time_old)/1000000000.0
        self.time_old = time.monotonic_ns()
        ...
        stepQuat = Quaternion(T(step)[0], T(step)[1], T(step)[2], T(step)[3])

        gyroscopeQuat = gyroscopeQuat + (q.conj() * stepQuat) * 2 * self.samplePeriod * self.zeta * -1

        # Compute rate of change of quaternion
        qdot = (q * gyroscopeQuat) * 0.5 + (-self.beta * T(step))
        
        # Integrate to yield quaternion
        q += qdot * self.samplePeriod
        self.quaternion = Quaternion(q._q / norm(q._q))  # normalise quaternion
    ...
    def update_imu(self, gyroscope, accelerometer):
        ...
        # Compute rate of change of quaternion
        qdot = (q * Quaternion(0, gyroscope[0], gyroscope[1], gyroscope[2])) * 0.5 + (-self.beta * T(step))
